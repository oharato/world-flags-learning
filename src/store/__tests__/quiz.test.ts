...
describe('Quiz Store', () => {
  ...
  describe('setupQuiz', () => {
    it('クイズ設定を正しく保存する', () => {
      ...
      countriesStore.countries = [
        { id: '1', name: '日本', capital: '東京', continent: 'アジア', flag_image_url: '', map_image_url: '', description: '', summary: '' },
        { id: '2', name: 'アメリカ', capital: 'ワシントンD.C.', continent: '北アメリカ', flag_image_url: '', map_image_url: '', description: '', summary: '' },
        { id: '3', name: 'フランス', capital: 'パリ', continent: 'ヨーロッパ', flag_image_url: '', map_image_url: '', description: '', summary: '' },
      ];
  ...
  describe('generateQuestions', () => {
    ...
    it('地域でフィルタリングされた問題を生成する', () => {
      ...
      countriesStore.countries = [
        { id: '1', name: '日本', capital: '東京', continent: 'アジア', flag_image_url: '', map_image_url: '', description: '', summary: '' },
        { id: '2', name: '中国', capital: '北京', continent: 'アジア', flag_image_url: '', map_image_url: '', description: '', summary: '' },
        { id: '3', name: 'アメリカ', capital: 'ワシントンD.C.', continent: 'North America', flag_image_url: '', map_image_url: '', description: '', summary: '' },
      ];

      store.setupQuiz('Test', 'flag-to-name', 'Asia', 5);

      store.questions.forEach(question => {
        const normalizedContinent = question.correctAnswer.continent === 'アジア' ? 'Asia' : question.correctAnswer.continent;
  ...
  describe('answerQuestion', () => {
    ...
    it('不正解時に正解数が増加しない', () => {
      const store = useQuizStore();
      const wrongId = store.questions[0]!.options.find(
        o => o.id !== store.questions[0]!.correctAnswer.id
      )!.id;
