...
- [x] ログ取得と解析
    ...
    - [x] バックエンドログ取得
        - [x] APIミドルウェアでアクセスログを記録（IP、国、ブラウザ、ステータスコード、レスポンス時間）
        - [x] エラーハンドリングミドルウェアでエラーログを記録（エラーメッセージ、スタックトレース、IP、国、ブラウザ、ステータスコード）
    - [x] フロントエンドログ取得
        - [x] Cloudflare Web Analytics が SPA 遷移を自動追跡
        - [x] グローバルエラーハンドラーでフロントエンドエラーをキャプチャ
    - [x] プライバシーポリシーページの作成
        - [x] PrivacyPolicy.vue コンポーネントを作成
        - [x] ルーターに /privacy-policy パスを追加
        - [x] フッターからリンク
    - [x] ドキュメントの作成
        - [x] docs/12_logging_analytics.md を作成
- [ ] 機能拡張
    - [ ] ユーザー登録・ログイン機能
    - [ ] 学習進捗の記録・可視化
    - [ ] ソーシャルシェア機能
