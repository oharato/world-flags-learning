...
- [x] 6.7. コードリファクタリング
    ...
    - [x] テスト設定の最適化
- [x] 6.8. テストカバレッジの確認 (163テスト、100%合格率達成)
- [x] 6.9. セキュリティ機能のテスト
    - [x] レート制限のテスト (10テスト)
    - [x] スコア検証のテスト (15テスト)
...
- [x] ランキング登録エンドポイントのセキュリティ対策
    - [x] レート制限の実装（1分間に10リクエスト/IP）
    - [x] スコア検証の強化（正解数、時間、計算式の妥当性チェック）
    - [x] API スキーマの拡張（検証パラメータの追加）
    - [x] セキュリティテストの追加
    - [x] セッショントークンによるクイズフロー検証
        - [x] クイズ開始APIでセッショントークンを発行
        - [x] スコア登録時にセッショントークンを検証
        - [x] 問題セットと回答の一致を検証
        - [x] 経過時間の整合性を検証
    - [x] ドキュメントの更新
- [ ] 機能拡張
    - [x] ランキング登録をユーザー選択式に変更
        - [x] クイズ結果画面にランキング送信ボタンを追加
        - [x] 自動登録から手動登録に変更
        - [x] 登録完了メッセージの表示
        - [x] 翻訳（日本語・英語）の追加
    - [x] 世界地図クイズのPOC画面追加
        - [x] Leaflet.jsによる地図表示
        - [x] GeoJSONデータのロード（世界各国の国境）
        - [x] ラジオボタンによる国名選択機能
        - [x] 選択した国にズーム・フォーカス機能（fitBounds）
        - [x] 周辺国のホバーで国境・国名表示
        - [x] 地図の移動・拡大縮小機能
        - [x] ルーターに/map-quizパスを追加
        - [x] i18n翻訳の追加
        - [x] ホーム画面に地図クイズへのリンクを追加
    - [x] 世界地図クイズの本実装
        - [x] クイズ設定画面（MapQuizSetup.vue）
        - [x] 地図→国名モード（地図で正解の国が赤く表示、4つの国名から選択）
        - [x] 国名→地図モード（国名に対し4つの地図から正解を選択）
        - [x] CartoDB light_nolabels タイルで国名ラベルなしの地図表示
        - [x] クイズ結果画面（MapQuizResult.vue）
        - [x] クイズ結果画面に地図表示を追加
    - [x] 地図学習モードの実装（MapStudy.vue）
        - [x] フリップカードで地図と国名を表示
        - [x] 矢印キーで前へ/次へ、スペースキーで表裏切り替え
        - [x] 国名一覧から選択可能
        - [x] ホバー時に国名のツールチップ表示（学習モードのみ）
    - [x] 国名マスターデータの改善
        - [x] GeoJSON名から日本語/英語国名へのマッピングファイル生成スクリプト
        - [x] country-names-lookup.json の自動生成
        - [x] useCountryNameMap composable の改善
    - [ ] ユーザー登録・ログイン機能
    - [ ] 学習進捗の記録・可視化
    - [ ] ソーシャルシェア機能
    - [ ] 音声読み上げ機能（国名の発音）
